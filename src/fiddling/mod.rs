...
pub fn n_bits_by_index(bytes: &[u8], n_bits: u8, bit_idx: usize, bit_order: BitOrder) -> u64 {
    ...
    while n > 0 {
        if bit_order == MSBFirst {
            read_bits = read_bits * 256 + last_n_bits(reverse_bits(bytes[byte_idx]), n as u64) as u64;
        } else {
            let shift_bits = n_bits - n;
            read_bits = read_bits + ((first_n_bits(bytes[byte_idx], n as u64) as u64) << shift_bits);
        }
